...
!        call EXECUTE_COMMAND_LINE(moviecmd,exitstat=ierr,wait=.true.)
  ...
  ! If deltax = 0 and deltay ≠ 0, use the frame ratio to compute the former from the latter
  ...
  if(deltax_frame(proj_ind*2).eq.0d0 .and. deltay_frame(proj_ind*2).gt.0d0)then
     deltax_frame(proj_ind*2)=deltay_frame(proj_ind*2)*float(nw_frame)/float(nh_frame)
  endif
  ! Same as above, but setting deltay from deltax
  ...
  if(deltay_frame(proj_ind*2).eq.0d0 .and. deltax_frame(proj_ind*2).gt.0d0)then
     deltay_frame(proj_ind*2)=deltax_frame(proj_ind*2)*float(nh_frame)/float(nw_frame)
  endif
  ...
  if(myid==1) then
     ...
     write(*, '(A,3F10.6)', advance="no")      ' xc=', xcen, ycen, zcen
...
subroutine set_movie_vars()
  ...
  do kk=1,n_movie_vars
     if(movie_vars_txt(kk) .eq. 'dm') then
        ...
        read( movie_vars_txt(kk)(4:), '(i10)' ) ivar
        ...
        read( movie_vars_txt(kk)(3:), '(i10)' ) ivar
        movie_var_number(kk) = ivar
